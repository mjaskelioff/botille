...
- **Launcher args go to the container command**, not to `podman run` flags. `nix run .# -- foo` runs `foo` inside the container (overriding default CMD `/bin/bash`). Exceptions: `--allow-lan`, `--devshell`, `--host-port`, `--port`/`-p`, `--share-claude`, `--volume`/`-v` are consumed by the launcher. Runtime flags layer on top of declarative config from `extraContainerModules`.
